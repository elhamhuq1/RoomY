...
plan: 02
subsystem: ui
tags: [react-native, nativewind, supabase, rpc, dispute, swap, dashboard, expo-router]

# Dependency graph
requires:
  - phase: 03.1-chores plan 01
    provides: chores, chore_completions, chore_swap_requests tables, RPC functions, chore list screen, TypeScript interfaces
  - phase: 01-foundation
...
tech-stack:
  added: []
  patterns:
    - "Client-side dispute auto-revert fallback (queries stale disputes on screen load)"
    - "Modal member picker for swap requests (bottom sheet style)"
    - "Segmented control toggle for time period filtering"
    - "Relative progress bars (proportional to max completion count)"
...
requirements-completed: [CHOR-01, CHOR-02, CHOR-04]
