...
plan: 03
subsystem: ui
tags: [react-native, nativewind, supabase-rpc, venmo, deep-link, expo-linking, settlement]

# Dependency graph
requires:
  - phase: 02-expense-splitting
    provides: "expenses, expense_splits, settlements tables, get_household_balances() function, TypeScript types"
provides:
...
tech-stack:
  added: []
  patterns:
    - "supabase.rpc('get_household_balances') for computed net balances per roommate"
    - "Linking.openURL with HTTPS Venmo URL for Expo Go compatibility"
    - "useFocusEffect to refetch balances on screen focus (returning from settle/add)"
    - "Mark as settled flow after returning from Venmo app"
...
requirements-completed: [EXPN-03, EXPN-05, EXPN-06]
