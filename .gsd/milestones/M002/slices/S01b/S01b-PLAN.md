...
- Diagnostic: RPC returns `split_mode` field — verify `'ownership'` when items are assigned, `'even'` when no assignments provided
- Failure path: assign-items screen with empty receiptItems shows empty state with "Go Back" action (no crash)

## Tasks

- [x] **T01: Add assigned_to column and update RPC for ownership-based splits** `est:45m`
...
- [x] **T02: Build item assignment screen** `est:1h15m`
...
- [x] **T03: Rewire navigation and update complete-trip for ownership splits** `est:1h`
...
- RPC returns `split_mode` field (`'ownership'` or `'even'`) — callers can log/display which split path was used
- `assigned_to` column on `grocery_items` is queryable per-trip: `SELECT name, assigned_to, unit_price FROM grocery_items WHERE trip_id = ?` to audit item ownership after completion
- Unassigned items (assigned_to IS NULL) are visible in trip queries — indicates items that fell through to even-split
- Index `idx_grocery_items_assigned_to` supports efficient per-user item history queries

## Failure-Path Verification

- RPC with `p_item_assignments` containing a non-existent user UUID: FK constraint on `assigned_to` will reject the UPDATE (transaction rolls back, no partial state)
- RPC with empty `p_item_assignments` (NULL or empty array): falls back to even split — backward compatible
- RPC with assignments but no `unit_price` on items: `v_assigned_total = 0`, falls back to even split — no divide-by-zero
